...
class MaskedAutoencoderLIT(LightningModule):
    def __init__(
        self,
        size: str = 'base',
        in_chans: int = 12,
        ...
        if self.size == 'base':
            self.model = mae_vit_base_patch16_dec512d8b(in_chans=in_chans)
        elif self.size == 'large':
            self.model = mae_vit_large_patch16_dec512d8b(in_chans=in_chans)
        elif self.size == 'huge':
            self.model = mae_vit_huge_patch14_dec512d8b(in_chans=in_chans)
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        if batch_idx == 0:
            full_grid = self.generate_grid_of_samples(x, pred, mask)
...
def mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        ...
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), norm_pix_loss=False,**kwargs)
    return model
